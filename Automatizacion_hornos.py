...
def crear_y_guardar_hoja(wb, df_base: pd.DataFrame, nombre_hoja: str, columnas_destino: list, fill_encabezado: PatternFill, font_negrita: Font, hoja_salida_name: str = None):
    """Crea y guarda una hoja de cálculo con vinculación dinámica universal."""
    ...
    if not df_a_guardar.empty:
        # Mapeo de letras de columnas en la hoja procesada de origen
        ...
        for r_idx, (df_idx, _) in enumerate(df_a_guardar.iterrows(), start=2):
            fila_origen = df_idx + 2 # +1 encabezado, +1 base 0
            
            for c_idx, col_nombre in enumerate(columnas_destino, start=1):
                # Caso: Campos fijos determinados
                ...
                # Caso: Rechazo anterior vacío y Diferencia local
                if nombre_hoja == COL['HOJA_SALIDA_RECHAZO']:
                    if col_nombre == '% rechazo anterior':
                        continue # Queda vacío
                    if col_nombre == COL['DIFERENCIA']:
                        idx_actual = columnas_destino.index(COL['PORCENTAJE_RECHAZO']) + 1
                        idx_anterior = columnas_destino.index('% rechazo anterior') + 1
                        l_actual = get_column_letter(idx_actual)
                        l_anterior = get_column_letter(idx_anterior)
                        ws.cell(row=r_idx, column=c_idx, value=f"={l_actual}{r_idx}-{l_anterior}{r_idx}")
                        continue

                # Caso: Vinculación universal por fórmula
                if col_nombre in mapeo_columnas_origen:
                    ...
                    formula = f"=IF({referencia}=0,\"\",{referencia})"
                    ws.cell(row=r_idx, column=c_idx, value=formula).number_format = '#,##0.00'

    # Formato de Encabezados (Colores Originales)
...
def automatizacion_final_diferencia_reforzada(file_original: io.BytesIO, file_info_externa: io.BytesIO, nombre_horno: str) -> Tuple[bool, Union[str, io.BytesIO]]:
    ...
    try:
        ...
        fill_encabezado = PatternFill(start_color='DDEBF7', end_color='DDEBF7', fill_type='solid') # Azul
        ...
        for r in range(2, len(df_original_final) + 2):
            ws.cell(row=r, column=col_dif_idx, value=f'=ROUNDDOWN({col_base_letter}{r}, 0) - {col_calculada_letter}{r}')
            sum_expr = f'SUM({",".join([f"{l}{r}" for l in col_sum_letters])})'
            ws.cell(row=r, column=col_suma_valores_idx, value=f'=IF({sum_expr}=0,"",{sum_expr})')
        ...
        # Crear Hojas Adicionales Vinculadas
...
def main():
    ...
    with col2:
        file_externa = st.file_uploader("Carga el archivo externo", type=['xlsx', 'xlsb'])
